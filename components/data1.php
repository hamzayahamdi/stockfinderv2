...
if ($type === 'category' || ($type === 'search' && !empty($query))) {
    ...
    if ($type === 'category') {
        if ($query === 'Bouskoura-Temp') {
            // Special query for Bouskoura-Temp
            $sql = "SELECT p.ref AS `Ref. produit`, 
                    p.label AS `Libellé`, 
                    p.price_ttc AS `Prix Promo`,
                    ps.reel AS `Total Stock`
                    FROM llx_product p
                    ...
                    WHERE ps.fk_entrepot = 23
                    AND ps.reel > 0
                    AND c.label NOT IN ('OLD', 'ABDELAZIZ ALAIDI', 'AIT MAZIGHT', 'ANI REDOUANE', 'AZTOT NASER', 'BAMOU MOHAMED', 'BELFQIH JAWAD', 'DARIF MUSTAPHA', 'FIKRI MOHAMED', 'GAYAL', 'NASR RACHID', 'NIAMI ABDELKEBIR', 'OUHAJJOU ABDELLATIF', 'OUTDOORZ', 'SAOUD MOHAMED', 'TAOUDI SLAOUI', 'NAQRAOUI AZEDINE', 'SAID AKCHOUCH', 'BADDA ABDELLATIF')
                    ORDER BY ps.reel DESC";
            ...
            $sql = "SELECT p.ref AS `Ref. produit`, p.label AS `Libellé`, p.price_ttc AS `Prix Promo`,
                    ...
                    LEFT JOIN llx_categorie_product cp ON p.rowid = cp.fk_product
                    LEFT JOIN llx_categorie c ON cp.fk_categorie = c.rowid
                    WHERE c.label = :query AND c.label NOT IN ('OLD', 'ABDELAZIZ ALAIDI', 'AIT MAZIGHT', 'ANI REDOUANE', 'AZTOT NASER', 'BAMOU MOHAMED', 'BELFQIH JAWAD', 'DARIF MUSTAPHA', 'FIKRI MOHAMED', 'GAYAL', 'NASR RACHID', 'NIAMI ABDELKEBIR', 'OUHAJJOU ABDELLATIF', 'OUTDOORZ', 'SAOUD MOHAMED', 'TAOUDI SLAOUI', 'NAQRAOUI AZEDINE', 'SAID AKCHOUCH', 'BADDA ABDELLATIF')
        ...
        $sql = "SELECT p.ref AS `Ref. produit`, p.label AS `Libellé`, p.price_ttc AS `Prix Promo`,
                ...
                LEFT JOIN llx_categorie_product cp ON p.rowid = cp.fk_product
                LEFT JOIN llx_categorie c ON cp.fk_categorie = c.rowid
                WHERE " . implode(' AND ', $conditions) . " AND c.label NOT IN ('OLD', 'ABDELAZIZ ALAIDI', 'AIT MAZIGHT', 'ANI REDOUANE', 'AZTOT NASER', 'BAMOU MOHAMED', 'BELFQIH JAWAD', 'DARIF MUSTAPHA', 'FIKRI MOHAMED', 'GAYAL', 'NASR RACHID', 'NIAMI ABDELKEBIR', 'OUHAJJOU ABDELLATIF', 'OUTDOORZ', 'SAOUD MOHAMED', 'TAOUDI SLAOUI', 'NAQRAOUI AZEDINE', 'SAID AKCHOUCH', 'BADDA ABDELLATIF')
                GROUP BY p.ref, p.label
                ORDER BY `Total Stock` DESC";
