...
import { FiX, FiClock, FiSearch, FiChevronDown, FiArrowUp, FiArrowDown, FiPlus, FiTag, FiPackage } from 'react-icons/fi';
...
interface PrixPromoHistoryProps {
  isOpen: boolean;
  onClose: () => void;
}

type TabType = 'promo' | 'pack';
...
export const PrixPromoHistory: React.FC<PrixPromoHistoryProps> = ({
  isOpen,
  onClose,
}) => {
  ...
  const fetchPriceChanges = async () => {
    ...
    setError(null);
    try {
      const endpoint = activeTab === 'promo' 
        ? 'update_price_api.php'
        : 'black_friday_api1.php';

      const response = await fetch(`https://phpstack-937973-4763176.cloudwaysapps.com/${endpoint}`, {
        method: 'POST',
        headers: {
