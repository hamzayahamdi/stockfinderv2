...
export function StockIndexComponent() {
  ...
  if (!isAuthenticated) {
    return <PasswordEntry onAuthenticated={() => setIsAuthenticated(true)} />
  }
    ...
    <div className="flex h-screen bg-gray-100 font-sans uppercase">
      ...
      >
        ...
        <div className="mt-auto p-4">
          <Button onClick={handleLogout} variant="outline" className="w-full text-white border-white hover:bg-white hover:text-[#144271]">
            Logout
          </Button>
        </div>
      </div>
                    ...
                    <tr className="bg-gray-100 text-gray-600 uppercase">
                      {table.getHeaderGroups().map((headerGroup) => (
                        headerGroup.headers.map((header) => (
                          <th key={header.id} className="px-2 py-3 font-semibold tracking-wider text-left">
                        ...
                        {row.getVisibleCells().map((cell) => {
                          ...
                          return (
                            <td 
                              key={cell.id} 
                              style={cellStyle} 
                              className={`px-2 py-2 ${isMobile ? 'break-words' : 'whitespace-nowrap'}`}
                            >
